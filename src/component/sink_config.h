...
namespace DslPipelineServer
{
    ...
    typedef enum SinkCompType
    {
        ...
        V4L2_SINK_COMP_TYPE                            = 1,      // streams video to a V4L2 device or v4l2loopback
        FILE_SINK_COMP_TYPE                            = 2,      // encodes video to a media container file
        RECORD_SINK_COMP_TYPE                          = 3,      // similar to the File sink but with Start/Stop/Duration control and a cache for pre-start buffering
        RTMP_SINK_COMP_TYPE                            = 4,      // streams encoded video using the Real-time Messaging Protocol (RTMP) to social media networks, live streaming platforms, and media servers
        RTSP_CLIENT_SINK_COMP_TYPE                     = 5,      // streams encoded video using the Real-time Streaming Protocol (RTSP) as a client of a media server
        RTSP_SERVER_SINK_COMP_TYPE                     = 6,      // streams encoded video via an RTSP (UDP) Server on a specified port
        MAX_SINK_COMP_TYPE,
    } SinkCompType;

    typedef struct AppSinkCompConfig
    {
    ...
    typedef struct SinkCompConfig : public BaseCompConfig
    {
        SinkCompType                                   sink_type;
        AppSinkCompConfig                              app_config;
